...
var kurlyTracker = (function(global) {
    ...
    var mobileOsApp = mobileOsAppCheck(global.appResult);
     ...
     * @param {object} window.appResult  
     * @returns {null|string} string I or A
     */
    function mobileOsAppCheck(app){
        if(typeof app === 'undefined' || typeof app.appCheck === 'undefined'){
            return null;
        }
        if(app && Number(app.verCheck[1]) >= 19){
            if(app.appDevice === 'I' && Number(app.verCheck[2]) >= 0){
                return app.appDevice;
            }
            if(app.appDevice === 'A' && Number(app.verCheck[2]) >= 2){
                return app.appDevice;
            }
        }
        return null;
    ...
    function saveSessionStorage(data, key) {
        ...
        // console.log('%c%s','font-size:16px;background-color:yellow', cmd);
        // console.table(data);
    ...
    /**
     * 헤더에 설정된 appResult 에서 production or development 설정 가져오기
     */
    function isProduction() {
        return appResult ? appResult.is_release_build : false
    }
        ...
        setAction: function(_event_name, _action){
            ...
            switch(_event_name){
                ...
                case 'view_sign_up' :
                    ...
                    break;
                // case 'select_category' : // 카테고리 항목 클릭
                // case 'order_creation_success' : // 주문서
                // case 'checkout_success' : // 결제하기
                // case 'purchase_product' : // 결제에 성공한 상품에 대한 정보
                // case 'select_search' : // 검색결과
                default : // _event_name 만 전달 하는 경우 : 회원 가입 성공, 카테고리, 검색, 메인이벤트
                    actionData = _action;
                    break;
            }
        ...
        sendData: function() {
            ...
            function sendAmplitude() {
                ...
                for(var key in bucketData){
                    ...
                    if(bucketData.screen_name !== BROWSE_TARGET.SIGN_UP && bucketData.screen_name !== BROWSE_TARGET.LOGIN) {
                        delete payload.sign_up_source_screen_name;
                    }
                }

                // console.log('%c%s','font-size:16px;background-color:#f00','amplitude 전달 데이터');
                // console.log('%c%s','font-size:16px;background-color:#f00', bucketData.event_name);
                // console.table(payload);
                // console.log('%c%s','font-size:16px;background-color:#f00','browseData 데이터');
...
var KurlyTracker = kurlyTracker.getTracker();
// module.exports = kurlyTracker;


/**
 * 해당 이벤트 발생시 KurlyTracker의 uuid 업데이트
 */
// var kurlyTrackerLogin = (function(){
//     function checkLogin(){
//         var type = null, name, msg;
//         try{
//             if(sessionStorage.getItem('login_fail') !== null){
//                 type = false;
//                 name = 'login_fail';
//                 msg = sessionStorage.getItem('login_fail');
//             }
//            
//             if(sessionStorage.getItem('login_success') !== null){
//                 type = true;
//                 name = 'login_success';
//                 msg = sessionStorage.getItem('login_success');
//             }
//            
//             if(type !== null){
//                 _set_login(name, msg);    
//             }
//         }catch(e){
//             console.log("ERROR on login SESSION calling!!!", e);
//         }        
//     }
//    
//     function _set_login(name, msg){
//         console.log(msg)
//         if(typeof msg !== 'undefined'){
//             KurlyTracker.setAction(name, msg).sendData();    
//         }else{
//             KurlyTracker.setAction(name).sendData();
//         }
//        
//         loginRemove(name);   
//     }
//    
//     function loginRemove(name){
//         sessionStorage.removeAttribute(name);
//     }
//    
//     return{
